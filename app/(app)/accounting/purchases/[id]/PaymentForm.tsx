...
export function PaymentForm({ purchaseId, totalAmount, paidAmount }: PaymentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/accounting/purchases/${purchaseId}`, {
        method: 'PATCH',
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      <h2 className="font-bold text-navy mb-4">지급 처리</h2>
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1">
          <label className="block text-sm font-medium text-navy">지급 상태</label>
          <select
