...
export default function SaleNewPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.title.trim()) { setError('매출명을 입력해주세요.'); return }
    if (!form.amount || Number(form.amount) <= 0) { setError('공급가액을 입력해주세요.'); return }
    if (!form.saleDate) { setError('매출일을 입력해주세요.'); return }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <h1 className="text-2xl font-bold text-navy">매출 등록</h1>
